...
build:
	...
	$(_V)$(SHELLCMD) stat --size-budget 8192 "$(BUILD)/ADJIROM_1fire.bin"
...
tom_laptop:
	$(_V)$(MAKE) build
	$(_V)$(SHELLCMD) blank-line
	curl --connect-timeout 0.25 --silent -G 'http://localhost:48075/reset/b2' --data-urlencode "config=Master 128 (MOS 3.50) ADJI test"
